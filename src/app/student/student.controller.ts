...
export class StudentController {
    ...
    async addCourse(@Param('studentId') studentId: number, @Param('courseId') courseId: number) {
        return (await this.studentService.addCourse(studentId, courseId)).toDTO();
    }

    @Delete('/:studentId/courses/:courseId')
    async removeCourse(@Param('studentId') studentId: number, @Param('courseId') courseId: number) {
        return (await this.studentService.removeCourse(studentId, courseId)).toDTO();
    }
